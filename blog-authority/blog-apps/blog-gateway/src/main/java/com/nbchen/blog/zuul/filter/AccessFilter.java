package com.nbchen.blog.zuul.filter;
...
import com.nbchen.blog.zuul.api.ResourceApi;
...
public class AccessFilter extends BaseFilter {
    ...
    public Object run() throws ZuulException {
        ...
        if(count == 0){
            ...
            errorResponse(ExceptionCode.UNAUTHORIZED.getMsg(), ExceptionCode.UNAUTHORIZED.getCode(),200); // 虽然授权失败,但是是业务上的异常,返回200
            return null;
        }
        ...
        if(visibleResource == null){
            ...
            if(resourceList != null && resourceList.size() > 0){
                visibleResource = resourceList.stream().map((resource -> resource.getMethod() + resource.getUrl())).collect(Collectors.toList());
                //将当前用户拥有的权限载入缓存
