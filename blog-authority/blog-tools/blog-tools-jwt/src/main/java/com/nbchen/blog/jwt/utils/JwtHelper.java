package com.nbchen.blog.jwt.utils;

import com.nbchen.blog.core.context.BaseContextConstants;
import com.nbchen.blog.core.exception.BizException;
import com.nbchen.blog.core.exception.code.ExceptionCode;
import com.nbchen.blog.core.utils.DateUtils;
import com.nbchen.blog.core.utils.NumberHelper;
import com.nbchen.blog.core.utils.StrHelper;
...
public class JwtHelper {
     ...
     * @throws BizException
     */
    public static Token generateUserToken(JwtUserInfo jwtInfo, String priKeyPath, int expire) throws
        BizException {
    ...
    protected static Token generateToken(JwtBuilder builder, String priKeyPath, int expire) throws BizException {
        try {
            //返回的字符串便是我们的jwt串了
            String compactJws = builder.setExpiration(
                    DateUtils.localDateTime2Date(LocalDateTime.now().plusSeconds(expire)))
                    //设置算法（必须）
